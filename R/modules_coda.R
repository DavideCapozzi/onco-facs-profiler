...
#' @param nPcs Number of components to estimate. Default is 3 (conservative for small N).
#' @return A numeric matrix with NAs filled.
#' @export
impute_matrix_bpca <- function(mat, nPcs = 3) {
  ...
  message(sprintf("   [Impute] Running Bayesian PCA (nPcs=%d) on %d x %d matrix...", 
                  nPcs, nrow(mat), ncol(mat)))
  
  tryCatch({
    # BPCA requires samples in rows, variables in columns (standard R)
    # pcaMethods expects a matrix/ExpressionSet
    res_bpca <- pcaMethods::pca(mat, method = "bpca", nPcs = nPcs, verbose = FALSE)
    mat_imputed <- pcaMethods::completeObs(res_bpca)
    
    # Validation: Check for crazy outliers generated by imputation
    # (Rare in BPCA but good defensive practice)
